...
import { getMorningRecipe } from '../morning-brew-recipes.mjs';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const recipe = getMorningRecipe(req.query?.recipe_id || req.query?.recipeId);
    ...
    if (query.length >= 2 && localMatches < 3) {
      ...
      if (provider === 'codex') return res.status(200).json({ query, recipe_id: recipe.id, sources: rankSources(sources, query, limit), catalog_sources: scopedCatalog, live: false, ranking_version: SOURCE_RANKING_VERSION, catalog_version: catalog.catalogVersion, updated_at: catalog.updatedAt });
    ...
    return res.status(200).json({ query, recipe_id: recipe.id, sources: rankSources(sources, query, limit), catalog_sources: scopedCatalog, live, ranking_version: SOURCE_RANKING_VERSION, catalog_version: catalog.catalogVersion, updated_at: catalog.updatedAt });
  } catch (error) {
