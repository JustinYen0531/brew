...
export async function discoverSources(query, { apiKey, model, limit = 5 } = {}) {
  if (!apiKey || String(query || '').trim().length < 2) return [];
  const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json', 'X-Title': 'Vibe Coding Source Discovery' },
    body: JSON.stringify({
      model,
      messages: [
  ...
  const text = extractText(message.content) || extractText(message.reasoning);
